...
```bash
cp .env.example .env
docker compose up -d                # LocalStack + MLflow + FastAPI (:8000) + Ollama (1st run pulls ~2GB qwen2.5:3b)
...
against the `ollama` service (it falls back to a threshold decision while the model is still pulling).
